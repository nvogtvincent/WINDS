...
for code in code_list:
    # Create empty LPDF
    lpdf[code] = np.zeros((len(lat), len(lon)), dtype=float)

    # Firstly, create a PDF based on the virtual drifter trajectories
    traj_fh_list = sorted(glob(dirs['traj'] + 'WINDS_FLOAT_' + code + '*.nc'))

    # Total drifters
    tot_drifter = 0

    for traj_fh in tqdm(traj_fh_list, total=len(traj_fh_list)):
        ...
        lpdf[code] += freq_matrix.sum(axis=-1).T

        # Count number of drifters
        tot_drifter += len(traj.idx0)

    # Convert to proportion of drifters
    lpdf[code] = lpdf[code]/tot_drifter
    tot_drifters[code] = tot_drifter
...
for ax_pos, code in enumerate(code_list):
    ...
    pcm = ax[ax_pos].pcolormesh(lon_bnd, lat_bnd, lpdf[code],
    ...
    for gdp_id in gdp_id_list:
        ...
        # Plot
        ax[ax_pos].plot(lon_subset, lat_subset, linewidth=0.25, color='k',
    ...
    if code in ['MAH', 'CHA']:
        ax[ax_pos].text(76.9, -23.0, str(n_drifters) + ' GDP drifters', va='bottom', ha='right', fontsize=10)
        ax[ax_pos].text(76.9, -21.4, str(int(tot_drifters[code]/1e3)) + 'k virtual drifters', va='bottom', ha='right', fontsize=10)
    else:
        ax[ax_pos].text(76.9, -0.5, str(n_drifters) + ' GDP drifters', va='top', ha='right', fontsize=10)
        ax[ax_pos].text(76.9, -2.1, str(int(tot_drifters[code]/1e3)) + 'k virtual drifters', va='top', ha='right', fontsize=10)

cb = plt.colorbar(pcm, cax=ax[-1], fraction=0.05, orientation='horizontal')
cb.set_label('Fraction of virtual drifters passing through cell', size=14)
ax[-1].tick_params(axis='x', labelsize=12)

plt.savefig(dirs['fig'] + 'GDP_validation.pdf', bbox_inches='tight', dpi=300)
